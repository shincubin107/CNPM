...
namespace QuanliBanHang
{
    public partial class formQuanLiBanHang : Form
    {
        ...
        void HienMENU()
        {
            SqlConnection connection = new SqlConnection(connectionSTR);
            connection.Open();
            string query = "select TenFoodDrink as [Món Ăn], TenDanhMuc as [Danh Mục], Gia as [Giá Tiền] from FoodDrink where TrangThai='Sale' ";
            da = new SqlDataAdapter(query, connection);
            DataTable dt = new DataTable();
            da.Fill(dt);

            dataGridViewMenu.DataSource = dt;
            dataGridViewMenu.Columns[0].Width = 210;
            dataGridViewMenu.Columns[1].Width = 160;
            dataGridViewMenu.Columns[2].Width = 150;
        ...
        private void comboBoxDanhMucItems_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string query = "select TenFoodDrink as [Món Ăn], TenDanhMuc as [Danh Mục], Gia as [Giá Tiền] from FoodDrink where TenDanhMuc = N'" + item + "'and TrangThai='Sale' ";
            SqlConnection connection = new SqlConnection(connectionSTR);
            connection.Open();
            da = new SqlDataAdapter(query, connection);
            DataTable dt = new DataTable();
            ...
            if (item != "")
            {
                dataGridViewMenu.Columns[0].Width = 210;
                dataGridViewMenu.Columns[1].Width = 160;
                dataGridViewMenu.Columns[2].Width = 170;
                dataGridViewMenu.Columns[2].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            }
            ...
            {               
                ...
                {                   
                    ...
                    if (cmdKiemTraIdRong.ExecuteScalar() != null)
                    {
                        ...
                        cmdThemBill.Parameters.AddWithValue("NhanVienLap", NhanVienThanhToan);
                        cmdThemBill.ExecuteNonQuery();

                        //Thêm vào chi tiết bill
                        for (int sodong = 0; sodong < count; sodong++)
                        {
                            string queryThemChiTietBill = "insert into ChiTietBill(IdBill,TenFoodDrink_CTB,SoLuong,DonGia_CTB) values (@IdBill,@TenFoodDrink_CTB,@SoLuong,@DonGia_CTB)";
